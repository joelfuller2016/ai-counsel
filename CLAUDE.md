...
14. **Cross-Platform Path Handling**: Always use `os.pathsep` (not `:`) for PATH-like string splitting, `os.sep` or `pathlib.Path` for file paths. Windows backslashes in JSON require escaping (`path.replace("\\", "\\\\")`) or JSON parsing fails with unicode escape errors. Use `sys.platform == "win32"` for platform detection and `USERPROFILE` (Windows) vs `HOME` (Unix) for home directories.
