...
class TestTechnicalDecisionsQuality:
    ...
    async def test_technical_reasoning_depth(self, engine, technical_test_cases):
        ...
        for test_case in technical_test_cases:
            ...
            print(
                f"📝 Response Length: {reasoning_scores[-1]['response_length']} chars"
            )

        # Overall technical reasoning assessment
        avg_depth = sum(s["depth_score"] for s in reasoning_scores) / len(
            reasoning_scores
        )
        ...
        print(
            f"   • High-Quality Responses: {sum(1 for s in reasoning_scores if s['depth_score'] >= 0.7)}/{len(reasoning_scores)}"
        )
    ...
    async def test_context_awareness(self, engine, technical_test_cases):
        ...
        for test_case in technical_test_cases:
            ...
            for context_variant in contexts:
                ...
                result = await engine.execute(request)
                context_score = self._assess_context_relevance(
                    result, test_case, context_variant
                )
                ...
                print(
                    f"--- {test_case['name']} ({context_variant.split('-')[-1].strip()}) ---"
                )
                print(f"🎯 Context Relevance: {context_score:.2f}/1.00")

        avg_context = sum(s["context_score"] for s in context_scores) / len(
            context_scores
        )
    ...
    async def test_practical_recommendations(self, engine, technical_test_cases):
        ...
        avg_practicality = sum(
            s["practicality_score"] for s in practicality_scores
        ) / len(practicality_scores)
    ...
    async def _assess_technical_depth(self, result, test_case):
        ...
        if test_key in technical_terms:
            ...
            tech_term_score = min(
                found_terms / 3, 1.0
            )  # Require at least 3 technical terms
        ...
        tradeoff_score = sum(
            1 for indicator in tradeoff_indicators if indicator in response
        )
    ...
    def _assess_context_relevance(self, result, test_case, context):
        ...
        matched_context = sum(
            1 for kw in context_keywords if kw in context_lower and kw in response
        )
        ...
        if "5 developers" in context_lower or "small team" in context_lower:
            if any(
                term in response
                for term in ["learning curve", "simplicity", "onboarding"]
            ):
                score += 0.2
        elif "8 developers" in context_lower or "growing team" in context_lower:
            if any(
                term in response for term in ["collaboration", "standards", "process"]
            ):
                score += 0.2
    ...
    def _assess_practicality(self, result, test_case):
        ...
        if any(
            term in response
            for term in ["cost", "budget", "investment", "roi", "expensive"]
        ):
            score += 0.1
        ...
        if any(
            term in response
            for term in ["team", "learning", "training", "hiring", "expertise"]
        ):
            score += 0.1
