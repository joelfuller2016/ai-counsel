...
class TestLocalModelBenchmark:
    ...
    async def test_ollama_performance_benchmark(self, engine, legal_questions):
        ...
        for i, question_data in enumerate(legal_questions, 1):
            ...
            consensus_reached = (
                ...
                and result.convergence_info.status in convergence_statuses
            )
            metric = {
                "test_case": i,
                "duration": duration,
                ...
                "transcript_words": (
                    len(" ".join([r.response for r in result.full_debate]).split())
                    if result.full_debate
                    else 0
                ),
            }
            ...
            print(
                f"🔄 Convergence: {result.convergence_info.status if result.convergence_info else 'N/A'}"
            )
    ...
    async def test_lm_studio_performance_benchmark(self, engine, legal_questions):
        ...
        participants_template = make_participants(
    ...
    async def test_hybrid_local_deliberation_benchmark(self, engine, legal_questions):
        ...
        for i, question_data in enumerate(
            ...
            duration = time.time() - start_time
            convergence_status = (
                result.convergence_info.status if result.convergence_info else "unknown"
            )
            rounds_completed = (
                len(result.convergence_info.rounds)
                if result.convergence_info
                and hasattr(result.convergence_info, "rounds")
    ...
    def _evaluate_response_quality(self, result, question_data):
        ...
        if any(
            ...
            for indicator in ["consider", "therefore", "however", "additionally"]
        ):
            score += 0.2

        # Check for actionable advice
        if any(
            action in response for action in ["should", "recommend", "avoid", "ensure"]
        ):
            score += 0.2
    ...
    async def test_concurrent_local_models(self, engine, legal_questions):
        ...
        print(
            f"   • Success rate: {len([r for r in results if r.summary and r.summary.consensus])/len(results):.1%}"
        )

        # Verify all requests succeeded
        assert all(
            result.summary and result.summary.consensus for result in results
