...
class TestLocalVsCloudComparison:
    ...
    async def test_speed_comparison(self, engine, comparison_scenarios):
        ...
        model_participants = [
            {
                ...
                "participants": make_participants(
                    "ollama", "xingyaow/codeact-agent-mistral"
                ),
            },
            {
                "name": "LM Studio Local",
                "participants": make_participants(
                    "lmstudio", "llama-3-groq-8b-tool-use"
                ),
            },
            {
                "name": "Claude Cloud",
                "participants": make_participants("claude", "sonnet"),
            },
        ]
        ...
        for scenario in comparison_scenarios[:2]:  # Limit to 2 scenarios for time
            ...
            for model_info in model_participants:
                try:
                    ...
                    print(
                        f"✅ {model_info['name']}: {duration:.2f}s ({speed_results[scenario['name']][model_info['name']]['response_length']} chars)"
                    )
        ...
        for scenario_name, results in speed_results.items():
            ...
            if successful_models:
                ...
                print(
                    f"     • Speedup: {slowest[1]['duration']/fastest[1]['duration']:.1f}x"
                )
    ...
    async def test_cost_analysis(self, engine, comparison_scenarios):
        ...
        for usage in usage_scenarios:
            ...
            for model_name, cost_info in cost_estimates.items():
                ...
                cost_per_request = (
                    total_monthly / usage["requests_per_month"]
                    if usage["requests_per_month"] > 0
                    else 0
                )

                print(
                    f"   {model_name}: ${total_monthly:.2f}/month (${cost_per_request:.4f}/request)"
                )
        ...
        for usage in usage_scenarios:
            ...
            savings_percentage = (
                (savings / cloud_monthly) * 100 if cloud_monthly > 0 else 0
            )

            print(
                f"   {usage['name']}: ${savings:.2f} saved monthly ({savings_percentage:.1f}% reduction)"
            )
    ...
    async def test_quality_comparison(self, engine, comparison_scenarios):
        ...
        for scenario_name, results in quality_results.items():
            ...
            if successful_models:
                best_quality = max(
                    successful_models.items(), key=lambda x: x[1]["quality_score"]
                )
                avg_quality = sum(
                    r["quality_score"] for r in successful_models.values()
                ) / len(successful_models)

                print(f"   {scenario_name}:")
                print(
                    f"     • Best Quality: {best_quality[0]} ({best_quality[1]['quality_score']:.3f})"
                )
                ...
                medium_quality = [
                    name
                    for name, result in successful_models.items()
                    if 0.6 <= result["quality_score"] < 0.8
                ]
                ...
                if medium_quality:
                    print(
                        f"     • Medium Quality (0.6-0.79): {', '.join(medium_quality)}"
                    )
    ...
    async def test_privacy_security_comparison(self, engine, comparison_scenarios):
        ...
        privacy_scenario = [
            s for s in comparison_scenarios if s.get("privacy_sensitive", False)
        ][0]
        ...
        for model_name, features in privacy_features.items():
            ...
            for feature_name, feature_value in features.items():
                ...
                print(
                    f"   {status} {feature_name.replace('_', ' ').title()}: {feature_value}"
                )
                if feature_value:
                    privacy_score += 1
            ...
            if (
                privacy_scenario.get("privacy_sensitive", False)
                and features["data_leaves_premises"]
            ):
                print(
                    "   ⚠️  WARNING: Data leaves premises - not ideal for sensitive content"
                )
    ...
    async def test_hybrid_approach_benefits(self, engine, comparison_scenarios):
        ...
        for strategy in hybrid_strategies:
            ...
            hybrid_percentage = (
                scenario_distribution["hybrid"] / total_scenarios
            ) * 100

            estimated_cost_saving = (
                local_percentage / 100
            ) * 0.8  # 80% of typical cost when using local
            print(
                f"   📊 Distribution: Local {local_percentage:.0f}%, Cloud {cloud_percentage:.0f}%, Hybrid {hybrid_percentage:.0f}%"
            )
            print(
                f"   💰 Estimated Cost Savings: {estimated_cost_saving:.0%} vs all-cloud approach"
            )
    ...
    def _apply_hybrid_rule(self, rule, scenario):
        ...
        if rule == "privacy_sensitive ? local : cloud":
            ...
            return (
                "local"
                if scenario.get("priority") == "performance_critical"
                else "cloud"
            )
    ...
    def _assess_response_quality(self, result, scenario):
        ...
        content_score = sum(
            1 for indicator in content_indicators if indicator in response
        )
        ...
        context_match = sum(
            1 for word in context_lower.split() if len(word) > 4 and word in response
        )
