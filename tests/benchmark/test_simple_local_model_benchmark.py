...
class TestSimpleLocalModelBenchmark:
    ...
    async def test_ollama_basic_performance(self, engine):
        ...
        try:
            ...
            print(
                f"   • Response length: {len(consensus_text) if consensus_text else 0} chars"
            )
    ...
    async def test_simple_quality_assessment(self, engine):
        ...
        for test_case in test_cases:
            ...
            if hasattr(result, "full_debate") and result.full_debate:
                ...
                print(
                    f"   🗳️  Voting result: {'Available' if result.voting_result else 'None'}"
                )
            else:
    ...
    async def testCostComparisonDemonstration(self, engine):
        ...
        for scenario in cost_scenarios:
            ...
            print(
                f"   💸 Cloud model estimate: ${scenario['cloud_estimated_cost']:.2f}"
            )
            print(f"   💵 Savings: ${savings:.2f}")
            print(
                f"   📝 Response: {len(consensus_text) if consensus_text else 0} chars"
            )
        ...
        print(
            f"📈 PROJECTED ANNUAL SAVINGS (100 requests/mo): ${total_savings * 12 * 100:.2f}"
        )
    ...
    def _assess_simple_quality(self, result, test_case):
        ...
        if hasattr(result, "full_debate") and result.full_debate:
            full_debate_text = " ".join(
                [round_response.response for round_response in result.full_debate]
            ).lower()
