...
class TestToolRequest:
    ...
    def test_valid_tool_request_parsing(self):
        ...
        data = {"name": "read_file", "arguments": {"path": "/path/to/file.py"}}
    ...
    def test_invalid_tool_name_validation(self):
        ...
        data = {"name": "invalid_tool_123", "arguments": {}}
    ...
    def test_arguments_must_be_dict(self):
        ...
        data = {"name": "read_file", "arguments": "not a dict"}
        with pytest.raises(ValidationError):
            ToolRequest(**data)
...
class TestToolResult:
    ...
    def test_success_result_creation(self):
        ...
        result = ToolResult(
            tool_name="read_file", success=True, output="file contents here", error=None
        )
...
class TestToolExecutionRecord:
    ...
    def test_record_creation_with_metadata(self):
        ...
        result = ToolResult(
            tool_name="read_file", success=True, output="contents", error=None
        )
    ...
    def test_timestamp_auto_generation(self):
        ...
        record = ToolExecutionRecord(
            request=request, result=result, round_number=1, requested_by="test"
        )
        assert record.timestamp is not None
    ...
    def test_record_with_failed_tool_execution(self):
        ...
        result = ToolResult(
            tool_name="read_file", success=False, output=None, error="File not found"
        )
        record = ToolExecutionRecord(
            request=request, result=result, round_number=1, requested_by="test"
        )
